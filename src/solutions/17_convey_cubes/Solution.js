...
const part2 = (input) => {
  ...
  const isActive = (cube) => {
    return !!activeCubes.find(
      (c) =>
        c.x === cube.x && c.y === cube.y && c.z === cube.z && c.w === cube.w
    );
  };
  const getNeighbours = (cube) => {
    let neighbours = [];
    for (let i = cube.x - 1; i <= cube.x + 1; i++) {
      for (let j = cube.y - 1; j <= cube.y + 1; j++) {
        for (let k = cube.z - 1; k <= cube.z + 1; k++) {
          for (let l = cube.w - 1; l <= cube.w + 1; l++) {
            if (i === cube.x && j === cube.y && k === cube.z && l === cube.w)
              continue;
            neighbours.push({ x: i, y: j, z: k, w: l });
          }
        }
      }
    }
  ...
  input.forEach((row, y) => {
    row.split("").forEach((value, x) => {
      if (value === "#") activeCubes.push({ x: x + 1, y: y + 1, z: 0, w: 0 });
    });
  });
  for (let i = 0; i < 6; i++) {
    let nextStateActiveCubes = [];
